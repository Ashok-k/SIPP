...
char method_custom[MAX_HEADER_LEN];
#ifdef PCAPPLAY
...
*/   
   ...
   if (!auth_body) {
    ...
    sprintf(method_custom, "%s", src->getMethod());
    /* Determine the type of credentials. */
    char result[MAX_HEADER_LEN];
    if (strcmp(method_custom, "REGISTER")==0) {
    ...
    if (createAuthHeader(my_auth_user, my_auth_pass, method_custom, uri, auth_body, dialog_authentication,
...
void call::computeRouteSetAndRemoteTargetUri (char* rr, char* contact, bool bRequestIncoming)
{
   ...
   //LOG_MSG("\n\n---computeRouteSetAndRemoteTargetUri---\n\n");
  if (0 >=strlen (rr))
  {
    //
...
bool call::process_incoming(char * msg, struct sockaddr_storage *src)
{
    ...
    for(search_index = msg_index - 1;
	  ...
	  if (int checkTxn = call_scenario->messages[search_index]->response_txn) {
		...
		if (reply_code >= 100 && reply_code <= 199) {
		  ...
		  if (int ackIndex = transactions[checkTxn - 1].ackIndex) {
  ...
  if(((reply_code) ||
    ...
    */
    if ( (100 == reply_code) ||
	 (0 == reply_code) || // means this is a request.
...
/*  if (call_scenario->messages[search_index] -> bShouldRecordRoutes &&
  ...
  if ((call_scenario->messages[search_index] -> bShouldAuthenticate) && (reply_code == 200) and dialog_authentication) {
     char auth[MAX_HEADER_LEN];
     memset(auth, 0, sizeof(auth));
     strcpy(auth, get_header_content(msg, (char*)"Authentication-Info:"));
     if(auth[0] !=0){
        int new_nonce_pos=strstr(auth,"nonce")-auth;
        int old_nonce_pos=strstr(dialog_authentication,"nonce")-dialog_authentication;
        int index=0;
        while(index<strlen(strstr(auth,"nonce"))){
           dialog_authentication[old_nonce_pos+index]=auth[new_nonce_pos+index];
           index++;
        }
     }
  }
